...
db.borrowed.aggregate([
  ...
  { $unwind:"$book" },
  { $unwind:"$member" },
  { $project:{ borrow_id:1, date:1, returned:1, title:"$book.title", name:"$member.name" } }
])
...
db.borrowed.aggregate([
  ...
  { $unwind:"$book" },
  { $project:{ title:"$book.title", borrow_count:1 } }
])
...
db.borrowed.aggregate([
  ...
  { $unwind:"$member" },
  { $project:{ name:"$member.name", total_borrowed:1 } }
])
...
db.members.insertOne({ member_id:104, name:"Priya Sharma", joined_on:new Date("2024-07-01") })

 // 17. Identify books that have never been borrowed.
db.books.aggregate([
  { $lookup:{ from:"borrowed", localField:"book_id", foreignField:"book_id", as:"borrowed" } },
  { $match:{ borrowed:{ $size:0 } } },
...
date ).
db.borrowed.aggregate([
  { $group:{ _id:{ $dateToString:{ format:"%Y-%m", date:"$date" } }, total:{ $sum:1 } } },
  { $sort:{ _id:1 } }
])
...
db.borrowed.aggregate([
  ...
  { $project:{ borrow_id:1, "book.title":1, "book.copies":1, _id:0 } }
])
...
db.borrowed.find({ returned:false, due_date:{ $lt:new Date("2025-07-24") } }, { borrow_id:1, due_date:1, _id:0 })

 // Create a chart-style output showing how many books are borrowed per genre.
db.borrowed.aggregate([
  { $lookup:{ from:"books", localField:"book_id", foreignField:"book_id", as:"book" } },
  { $unwind:"$book" },
  { $group:{ _id:"$book.genre", total_borrowed:{ $sum:1 } } },
  { $project:{ genre:"$_id", total_borrowed:1, _id:0 } }
])
