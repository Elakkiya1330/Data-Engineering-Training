...
db.orders.aggregate([
  ...
  { $unwind: "$product_info" },
  { $project: { order_id: 1, customer: 1, category: "$product_info.category" } }
])
...
db.orders.aggregate([
  { $lookup: { from: "products", localField: "product_id", foreignField: "product_id", as: "product_info" } },
  { $unwind: "$product_info" },
  { $project: { customer: 1, total: { $multiply: ["$quantity", "$product_info.price"] } } },
...
db.orders.aggregate([
  { $lookup: { from: "products", localField: "product_id", foreignField: "product_id", as: "product_info" } },
  { $unwind: "$product_info" },
  { $project: { category: "$product_info.category", sale: { $multiply: ["$quantity", "$product_info.price"] } } },
  { $group: { _id: "$category", total_sales: { $sum: "$sale" } } }
])
...
db.orders.aggregate([
  { $lookup: { from: "products", localField: "product_id", foreignField: "product_id", as: "product_info" } },
  { $unwind: "$product_info" },
  { $project: { customer: 1, amount: { $multiply: ["$quantity", "$product_info.price"] } } },
  { $sort: { amount: -1 } },
  { $limit: 1 }
])
...
db.orders.aggregate([
  { $group: { _id: { $dateToString: { format: "%Y-%m-%d", date: "$order_date" } }, order_count: { $sum: 1 } } },
  { $sort: { order_count: -1 } },
  { $limit: 1 }
])
...
db.customers.insertOne({ name: "Aditi Rao" })
db.customers.aggregate([
  { $lookup: { from: "orders", localField: "name", foreignField: "customer", as: "order_info" } },
...
db.orders.insertMany([
  ...
  { order_id: 5008, customer: "Ravi Shah", product_id: 1005, quantity: 1, order_date: new Date("2024-07-08") }
])
...
db.products.aggregate([
  { $lookup:{ from:"orders", localField:"product_id", foreignField:"product_id", as:"order_info" } },
  { $match:{ order_info:{ $size:0 } } }
])
...
db.products.aggregate([
  ...
  { $group:{ _id:null, total_inventory_value:{ $sum:"$value" } } }
])
