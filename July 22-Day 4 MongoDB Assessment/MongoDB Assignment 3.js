...
db.sales.aggregate([
  ...
  { $project: { product_name: { $arrayElemAt: ["$product.name", 0] }, total_quantity: 1 } }
])
...
db.sales.aggregate([
  ...
  { $unwind: "$product" },
  { $project: { product_name: "$product.name", revenue: { $multiply: ["$quantity", "$product.price"] } } },
  { $group: { _id: "$product_name", total_revenue: { $sum: "$revenue" } } }
])
...
db.sales.aggregate([
  ...
  { $lookup: { from: "products", localField: "_id", foreignField: "product_id", as: "product" } },
  { $unwind: "$product" },
  { $project: { product_name: "$product.name", total_sold: 1 } },
  { $sort: { total_sold: -1 } },
  { $limit: 2 }
])